...
*/
		...
		*****************************/
        ...
        //             if(no2gnode[s][g].Count()) {
		// 				unispg_gp->AddGraph(fidx, s, unispg_gp->current_gidx+b, no2gnode[s]+b);
		// 			}
		// 		}
		// 	}
            ...
            for(int sno=0;sno<3;sno+=2) { // skip neutral bundles -> thoses shouldn't have junctions
                ...
                for(int g=0;g<bundle[sno].Count();g++) {
                    ...
                    if(no2gnode[s][g].Count()) {
						 ...
						 ****************/						
						// unispg_gp->AddGraph(fidx, s, unispg_gp->current_gidx, no2gnode[s]+g);



				// for (int i=0; i<no2gnode[s][g].Count(); i++) {
				// 	CGraphnode* node = no2gnode[s][g].Get(i);
				// 	unispg_gp->no2gnode_unispg[s][cgidx].Add(node);
				// 	fprintf(stderr, "Inside!! cgidx: %d \n", cgidx);
				// 	fprintf(stderr, "no2gnode->Get(i)->nodeid: %d \n", node->nodeid);
				// 	fprintf(stderr, "unispg_gp->no2gnode_unispg[s][0]->Last()->nodeid: %d \n", unispg_gp->no2gnode_unispg[s][0].Last()->nodeid);
				// 	if (cgidx >= 1) {
				// 		fprintf(stderr, "unispg_gp->no2gnode_unispg[s][cgidx-1]->Last()->nodeid: %d \n", unispg_gp->no2gnode_unispg[s][cgidx-1].Last()->nodeid);
				// 		fprintf(stderr, "unispg_gp->no2gnode_unispg[s][cgidx]->Last()->nodeid: %d \n", unispg_gp->no2gnode_unispg[s][cgidx].Last()->nodeid);
				// 		// fprintf(stderr, "unispg_gp->no2gnode_unispg[s][cgidx+1]->Last()->nodeid: %d \n", unispg_gp->no2gnode_unispg[s][cgidx+1].Last()->nodeid);
				// 	}
				// }
						...
						for(int nd=0;nd<no2gnode[s][g].Count();nd++) {
							...
							if (nd == 0) {
								...
								if(s == 0) {
									...
									fprintf(node_cov_neg_bed_vec.Get(fidx), "chr22\t%d\t%d\t%s\t%f\t%s\n", node_start, node_end, node_name.chars(), no2gnode[s][g][nd]->cov, strand_symbol.chars());
								} else if (s == 1) {
