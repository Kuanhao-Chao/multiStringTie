...
void processBundleUnispg(BundleData* bundle, UniSpliceGraph* unispg);
...
int main(int argc, char*argv[]) {
	...
	#define DEF_TSTACK_SIZE 8388608
		...
		fprintf(stderr, "unispgdotfname: %s\n", unispgdotfname.chars());
		...
		while (more_graph) {
			...
			if ((drec=dotreader.next())!=NULL) {
				...
				if (prev_r_start > drec->get_refend()) {
				...
				while (more_alns && read_in_unispg) {
					...
					if ((brec=bamreader.next())!=NULL) {
						...
						prev_r_start = brec->start; //start<end always!
						prev_r_end = brec->end;
