...
void infer_transcripts_APPLY_UNISPG(BundleData* bundle, GPVec<UnispgGp_APPLY>** graphs_vec) {
    ...
    for(int s=0;s<2;s++) { // skip neutral bundles -> those shouldn't have junctions
        ...
        if (g_num) {
            ...
            for (int g=0; g<g_num; g++) {
                ...
                tr2no[s][g]=construct_treepat_APPLY_UNISPG(graphs_vec[s]->Get(g)->node_nums[s][0], graphs_vec[s]->Get(g)->gpos[s][g], transfrag[s][g]);
    ...
    for(int s=0;s<2;s++) {
        ...
        for(int g=0;g<g_num;g++) {
            ...
            if (node_num) {
                process_transfrags_APPLY_UNISPG(s, node_num, edge_num, no2gnode[s][0], transfrag[s][g],tr2no[s][g],gpos[s][g],pred);
            }
        }
    }
