...
void processRead(int currentstart, int currentend, BundleData& bdata,
	...
	*/
	...
	bool longr=false;
	if(brec.uval) longr=true; // second alignment is always from mixed reads
...
void add_group_to_bundle(CGroup *group, CBundle *bundle, GPVec<CBundlenode>& bnode, uint localdist){
	/*****************************
	 * Step 1: get the current last node.
	 *****************************/
...
int merge_read_to_group(int n,int np, int p, float readcov, int sno,int readcol,GList<CReadAln>& readlist,int color,GPVec<CGroup>& group,
	...
	uint localdist=0;
	localdist=bundledist+longintronanchor;

	/*****************************
	 ** valid last/first exon and close
	 *****************************/
	if(np>-1 && np<n && readlist[np]->end<readlist[n]->start && readlist[np]->end+localdist>readlist[n]->start &&
			...
			&& (readlist[n]->segs[0].len()>=junctionsupport || (readlist[n]->juncs.Count() && readlist[n]->juncs[0]->strand))) {
			return color;
	}
	...
	/*****************************
	 ** this type of group - negative, unknown, or positive - was created before
	 *****************************/
	if(currgroup != NULL) { 
		...
		/*****************************
		 ** I need to split pairs here because I have no overlap 
		 ** => color didn't reach here for sure
		 *****************************/
		...
		while(i<ncoord) {
			...
			/*****************************
			 ** exon is too small to keep 
			 **  --> this exon will not be added to group if it doesn't pass some criteria
			 *****************************/
			...
			if(!i || keep) {
				...
				if(keep) {
					...
					/*****************************
					 ** add group to read so that I can remember what groups belong to a read
					 *****************************/	
	...
	while(bundlenode!=NULL) {
	    ...
	    do {
	    	...
	    	if(minjunction == 0 ) { // found a start junction here
	    		...
	    		// if(trim && !processguide) graphnode=trimnode(s,g,refstart,junction[njs]->start,graphnode,source,sink,bpcov,futuretr,graphno,bundlenode,bundle2graph,no2gnode,edgeno);// do something to find intermediate nodes; alternatively, I could only do this for end nodes
			...
			if(!hashnode[bnode]) {
				...
				while(j<nbnode && k<ncoord) {
					...
					if(intersect) { // read intersects gnode
						...
						/*****************************
						 ** add ngraph to list of graphs if not seen before
						 *****************************/
						if(g<0) { 
							rgno.Add(ngraph);
							g=rgno.Count()-1;
						...
						/*****************************
						 ** this is a new graph, or a new gnode
						 *****************************/
...
CTransfrag *update_abundance(int s,int g,int gno,GIntHash<int>&gpos,GBitVec& pattern,float abundance,GVec<int>& node,
	...
	*/
	...
	/*****************************
	 * Step 1: see if I need to adjust nodes in pattern
	 *   1. correct long-reads that extend inside introns or past nodes that link back to source or sink
	 *   2. Remove one node transfrag. CHECK IF YOU NEED TO KEEP THIS ONE
	 *****************************/
	if(is_lr) {
		if(node.Count()>1) { 
	...
	else if(node.Count()==1) return(NULL); // do the one node transfrag make any difference? CHECK IF YOU NEED TO KEEP THIS ONE

	/*****************************
	 * Step 2: find transfrag in the treepat
	 *****************************/
	...
	/*****************************
	 * Step 3: setting if the transfrag is long read or short read.
	 *****************************/
	...
	/*****************************
	 * Step 5: Setting longstart => for long reads: min start of all long-reads sharing transfrag
	 * 		   Setting longed => for long reads: max end of all long-reads sharing transfrag
	 *****************************/
	...
	*/
	...
	if(readlist[n]->nh && !readlist[n]->strand && np>-1 && readlist[np]->nh && !readlist[np]->strand) {
		...
		if(readlist[n]->nh) {
			...
			/*****************************
			 *  readlist[n] is deleted && the paired one exist.
			 *****************************/
...
void process_transfrags(int s, int gno,int edgeno,GPVec<CGraphnode>& no2gnode,GPVec<CTransfrag>& transfrag,CTreePat *tr2no,
	...
	*/
	...
	for(int t1=0;t1<transfrag.Count();t1++) { // transfrags are processed in increasing order -> important for the later considerations
		...
		fprintf(stderr, "n1: %d\n", n1);
   for (uint i=0;i<transfrag[t1]->pattern.size();i++) {
       fprintf(stderr, "%c", transfrag[t1]->pattern.test(i)?'1':'0');
   }
...
bool back_to_source_fast_long(int i,GVec<int>& path,int& minpath,int& maxpath,GBitVec& pathpat,GPVec<CTransfrag>& transfrag,GPVec<CGraphnode>& no2gnode,
	...
	fprintf(stderr, ">> Inside back_to_source_fast_long\n");
	// find all parents -> if parent is source then go back
	CGraphnode *inode=no2gnode[i];
...
bool fwd_to_sink_fast(int i,GVec<int>& path,GBitVec& pathpat,GPVec<CTransfrag>& transfrag,GPVec<CGraphnode>& no2gnode,
		GVec<float>& nodecov,int gno,GIntHash<int>& gpos){
	fprintf(stderr, ">> Inside fwd_to_sink_fast\n");
	// find all parents -> if parent is source then go back
	CGraphnode *inode=no2gnode[i];
...
bool back_to_source_fast(int i,GVec<int>& path,GBitVec& pathpat,GPVec<CTransfrag>& transfrag,GPVec<CGraphnode>& no2gnode,
		GVec<float>& nodecov,int gno,GIntHash<int>& gpos){
	fprintf(stderr, ">> Inside back_to_source_fast\n");
...
int store_guide_transcript(GList<CPrediction>& pred,GVec<int>& path,GVec<float>& nodeflux,GVec<float>& nodecov,
	...
	fprintf(stderr, ">> store_guide_transcript \n");
...
*/
	...
	if (eonly)
		...
		if(guidetrf.Count()) maxi=guides_pushmaxflow(gno,edgeno,gpos,no2gnode,transfrag,guidetrf,geneno,strand,pred,nodecov,istranscript,pathpat,first,guides,guidepred,bdata);
...
bool good_junc(CJunction& jd,int refstart, GVec<float>* bpcov) {
	...
	float mult=1/ERROR_PERC; // test if this is needed for long-reads
	int bw=5;
	...
	if(higherr) {
		...
		for(int i=1;i<junction.Count();i++) { // junction is sorted based on start
			...
			if(junction[i]->strand) {
				...
				else if(junction[i]->nm<junction[i]->nreads) {
					jstarts.Add(i);
				}
			}
			...
			if(ejunction[i]->strand) {
				...
				else if(ejunction[i]->nm<ejunction[i]->nreads) {
					jends.Add(i);
				}
			}
		}
...
*/
    ...
    */
	...
	/*****************************
	 * Process groups. => either add them into bundles or create new ones.
	 *****************************/
	while(currgroup[0]!=NULL || currgroup[1]!=NULL || currgroup[2]!=NULL) { // there are still groups to process
...
int build_merge(BundleData* bdata) { // here a "read" is in fact a transcript
	...
	/*****************************
	 ** Process the read alignment by guides.
	 *****************************/
	if(guides.Count()) {
	...
	for (int n=0;n<nreads;n++) if(readlist[n]->nh){
		...
		//  Add the index itself to the pair_idx.
		rd.pair_idx.Add(n);

		/*****************************
		 ** if not guide : only collapse non guides
		 *****************************/
		if(rd.tinfo->g==-1) {
	...
	/*****************************
	 ** each unstranded group needs to remember what proportion of stranded group it overlaps so that it can distribute reads later on -> 
	 **    maybe I can do this in the following while?
	 *****************************/
	while(currgroup[0]!=NULL || currgroup[1]!=NULL || currgroup[2]!=NULL) { // there are still groups to process
...
int print_predcluster(GList<CPrediction>& pred,int geneno,GStr& refname,
	...
	excov*=abs(pred[0]->tlen); // it was len() before but that doesn't make any sense
	...
	for(int j=1;j<pred[0]->exons.Count();j++) {
		...
		if(introncov) {
		  if(introncov<1 || (introncov<singlethr)) intron[j-1]=1;
		  else {
			...
			for(int j=i+1;j<predord.Count();j++) if(pred[predord[j].predno]->flag) {
	    		...
	    		//else if(!pred[n2]->t_eq && pred[n2]->exons.Count()==1 && pred[n2]->cov<DROP*pred[n1]->cov && pred[n1]->start<pred[n2]->start && pred[n2]->end<pred[n1]->end) {
	    			...
	    			if(!pred[n1]->t_eq && pred[n1]->exons.Count()==1 && pred[n1]->cov<pred[n2]->cov && pred[n2]->start<=pred[n1]->start && pred[n1]->end<=pred[n2]->end) {
	    				...
	    				if(pred[n2]->flag) {
	    						if((pred[n1]->strand==pred[n2]->strand) && ((pred[n2]->cov<singlethr) || pred[n2]->cov<pred[n1]->cov*ERROR_PERC) && pred[n1]->start<=pred[n2]->start && pred[n2]->end<=pred[n1]->end && intronic(pred,n2,n1)) { // n2 is an intronic prediction to n1
	...
	*/
	...
	// recompute coverages -> mostly for short reads; long reads should be more reliable in the way coverages are estimated
	bool adjust=true;
	...
	for(int i=0;i<npred;i++) if(pred[i]->flag) {
		if(pred[i]->cov<1 || (!pred[i]->t_eq && (pred[i]->cov<readthr || guided && pred[i]->cov<singlethr))) {
			pred[i]->flag=false;
	...
	for(int n=0;n<npred;n++) if(pred[n]->flag){
		...
		{ // DEBUG ONLY
			  ...
			  fprintf(stderr, " exons= %d\n",pred[n]->exons.Count()+1);
...
uint find_midhash(int refstart,int start,int end,GVec<float>* bpcov) {
	...
	while(i<endcheck) {
		...
		else if(newcov==mincov) { // check if there is a better place to split; long-reads modification
